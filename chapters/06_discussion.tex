...
correction. The direction, however, is inconsistent across the panel, and the
...
taxonomy on clean born-digital text with expert ground truth. This axis is
...
run, in which a query (e.g.\ ``which contracts contain uncapped liability'')
is dispatched across the repository before per-contract extraction.
...
\paragraph{Schema-enforced grounding under no ground truth.}
...
triage. The structural auditability property the benchmark measures and
that no single-call configuration reproduces---not chain-of-thought
prompting and not reasoning-mode model variants, both of which emit
rationale tokens but no verbatim-citation guarantee---is schema-enforced
grounding. \cfgMA's $1.000$ verbatim-citation rate on every predicted clause
(Table~\ref{tab:grounding}, predicted-clause column) versus $0.543$ for
\cfgZS\ and $0.415$ for \cfgCP\ means that under M1, every emitted span is
locatable in the source contract by exact string match, and under B0/M2
roughly half are paraphrased or hallucinated. This is the one auditability
finding the benchmark supports without auxiliary measurement, and the
measured basis for preferring a multi-agent topology in deployments where
extraction provenance is a regulatory or contractual requirement
